use amethyst::{
	...
	audio::{output::Output, Source, Mp3Format, OggFormat, SourceHandle},
	ecs::{World, WorldExt},
};


const BACKGROUND_SOUND: &str = "demo_1.2.ogg";
...
pub fn play_background_sound(sounds: &Sounds, storage: &AssetStorage<Source>, output: Option<&Output>) {
	log::debug!("play_background_sound");
...
// pub fn play_with_audio_sink(sounds: &Sounds, storage: &AssetStorage<Source>, audio_sink: Option<&amethyst::audio::AudioSink>) {
// 	if let Some(sound) = storage.get(&sounds.background_sfx) {
// 		audio_sink.append(sound);
// 	}
// }
